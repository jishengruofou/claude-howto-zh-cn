...
如果你只是想临时加载一个本地或远程 plugin，上游这轮把命令也补全了：

```bash
# 从本地目录加载
claude --plugin-dir ./my-plugin

# --plugin-dir 也可以直接指向 .zip 包（v2.1.128+）
claude --plugin-dir ./my-plugin.zip

# 从 URL 临时拉取一个 plugin .zip（v2.1.129+，可重复指定）
claude --plugin-url https://example.com/releases/my-plugin-0.3.0.zip
```

这里的 `--plugin-dir`、`--plugin-url`、`.zip` 都属于可执行标识，不要中文化改写。
...
它们解决的是“安装 plugin 时自动带进来的依赖，父 plugin 删除后还残留”的问题。

- `claude plugin prune`：清理已经没有父 plugin 需要的自动安装依赖
- `claude plugin uninstall <name> --prune`：卸载目标 plugin 后顺手做级联清理

对团队环境来说，这能减少“装过一堆试验性 plugin 后环境越来越脏”的问题。
命令本身不要翻译，README 里只解释用途即可。
